...
// 挑选策略：healthy 账号中 API key 优先（有 api_key 优于无），再按 uid 排序。
package pool
...
const (
	CoolHard CoolKind = iota // 积分/余额不足 → 长冷却
	CoolSoft                 // 429 → 短冷却
	CoolErr                  // 连续错误 → 中冷却
)
...
// SyncToDir 用最新扫描结果对齐池：新账号加入、消失的剔除。
...
// Pick 返回 healthy 中 api_key 优先的账号；无可用返回 nil。
